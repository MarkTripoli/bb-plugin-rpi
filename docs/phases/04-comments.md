...
- Added the three native agent tools: `rpi_get_artifact_comments`, `rpi_update_artifact_comments`, and `rpi_reply_to_artifact_comment`, selected only for RPI task sessions through the existing synchronous `configure` callback.
- Added `sendCommentsMode` setting with default `send-and-resolve`.
- Added comment realtime publishes on create/reply/edit/update/delete/send and artifact badge refreshes through `rpi:artifacts`.
- Replaced artifact comment count semantics with unresolved root discussion count.
- Added CLI commands: `bb rpi comments list|create|resolve --task --file`.
...
- tool scoping error outside a RPI task session
...
The launched agent created `01-notes-live-comments.md` with three markdown paragraphs and saved it through `rpi_artifact_save`; artifact version 1 was visible through:

```text
bb rpi artifacts list --task 20b5cb36-2f44-457d-b47b-e673940ff9f1 --json
```

Added two comments through CLI:

```text
bb rpi comments create --task 20b5cb36-2f44-457d-b47b-e673940ff9f1 --file 01-notes-live-comments.md --block 0 --content "Please tighten the opening statement." --json
bb rpi comments create --task 20b5cb36-2f44-457d-b47b-e673940ff9f1 --file 01-notes-live-comments.md --block 1 --content "Please mark this paragraph as verified after review." --json
```

Steered the agent:

```text
bb thread tell thr_vwghqa93bm "Call rpi_get_artifact_comments on 01-notes-live-comments.md, reply to the first comment with 'acknowledged', and resolve the second comment."
...
```text
bb rpi comments list --task 20b5cb36-2f44-457d-b47b-e673940ff9f1 --file 01-notes-live-comments.md --resolved --json
```
...
- Added restore support for `deleted:false`, root-only resolve and unresolve failures for replies, server validation for artifact version ownership, reply roots, anchor block range, user edit ownership, and typed `rpi:comments` realtime event kinds.
...
Then `bb rpi comments list --resolved --json` showed roots `2fba98f4` and `6630b9f1` resolved, while root `cc15f804` remained unresolved.
