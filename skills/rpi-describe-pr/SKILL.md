...
description: Only use when the user explicitly invokes /rpi-describe-pr. Write a PR description.
---
...
2. If an artifact number is needed, call hl_next_artifact_number and use the returned number in NN-pr-description-short-slug.md.
3. Read references/pr_description_template.md and draft the artifact in that shape.
...
6. Read references/pr_description_final_answer.md and answer using that structure exactly. The final answer must end with one fenced text block containing /rpi-show-me.

## Rules

- Do not open unrelated task artifacts. Use only task.md, ticket.md, @ files, and comments the user asked you to inspect.
- Use bb environment status, diff-files, diff, and pull-request commands. Write pr-description.md; do not use gh as the default path.
