...
description: Only use when the user explicitly invokes /rpi-review-artifact-comments. Review artifact comments.
---
...
2. If an artifact number is needed, call hl_next_artifact_number and use the returned number in NN-comment-review-short-slug.md.
3. Read references/comments_template.md and draft the artifact in that shape.
...
6. Read references/comments_final_answer.md and answer using that structure exactly. The final answer must end with one fenced text block containing /rpi-iterate-implementation.

## Rules

- Do not open unrelated task artifacts. Use only task.md, ticket.md, @ files, and comments the user asked you to inspect.
- Ask the user what to do after reading comments unless they already told you. Work one root comment at a time. Resolve, delete, or reply only when instructed.
