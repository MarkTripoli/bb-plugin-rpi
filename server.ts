// bb-plugin-humanlayer — a BB plugin backend entry.
//
...
// surfaces: the Example todos page (app.tsx, over RPC), the `bb humanlayer` CLI
...
export default async function plugin(bb: BbPluginApi) {
  ...
  // `bb plugin config humanlayer`. Add `secret: true` for values like API keys.
  ...
  // The `bb humanlayer` command: what agents (and you) use from a shell. Parsing
  ...
  const usage = [
    "Usage:",
    "  bb humanlayer list [--json]",
    "  bb humanlayer add <title> [--json]",
    "  bb humanlayer done <todo-id> [--json]",
    "  bb humanlayer undo <todo-id> [--json]",
    "  bb humanlayer remove <todo-id> [--json]",
  ].join("\n");
  ...
  bb.cli.register({
    name: "humanlayer",
    summary: "Manage the Humanlayer plugin's example todo list",
    commands: [
      { name: "list", summary: "List todos", usage: "bb humanlayer list [--json]" },
      {
        name: "add",
        summary: "Add a todo",
        usage: "bb humanlayer add <title> [--json]",
      },
      {
        name: "done",
        summary: "Mark a todo done",
        usage: "bb humanlayer done <todo-id> [--json]",
      },
      {
        name: "undo",
        summary: "Mark a todo not done",
        usage: "bb humanlayer undo <todo-id> [--json]",
      },
      {
        name: "remove",
        summary: "Remove a todo",
        usage: "bb humanlayer remove <todo-id> [--json]",
      },
    ],
    async run(argv) {
      ...
      const notFound = (missingId: string) => ({
        exitCode: 1,
        stderr: `No todo with id ${missingId}. Run "bb humanlayer list" to see ids.`,
      });
      const todoId = args[0];
      switch (command) {
        case undefined:
