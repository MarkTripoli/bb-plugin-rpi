...
| UI surfaces | New task composer plus reviewable manual launch composer for Draft Launch, New chat, Start fresh, and header Iterate; phase-complete banner actions (Implement Phase N, Proceed, Review code, Create pull request, Resolve pull request reviews, Iterate) launch directly | partial | `ui/rpi.tsx` `NewTaskPage`/`ManualLaunchComposerPage`/`RpiComposerBanner`, `manual-launch.ts`, `plan-phases.ts` | Manual launch settings apply to one session. Task project and workspace are validated on submit; unmanaged paths use the SDK's normalized display but the server restores the canonical task path. BB persists prompt text by `draftKey`, but its public embedded composer keeps execution and environment selections only in component-local state, so those selections reset to task seeds after Back and reopen. CLI launches, lifecycle auto-advance, and phase-complete banner actions remain immediate. |
...
| UI surfaces | One RPI thread panel hub: Artifacts, Workspace, Scratch pad, Tips, Minimap, model, auto-advance, task tools, and launch-attempt recovery | full (superset) | `app.tsx` `threadPanelAction`, `ui/rpi.tsx` `RpiThreadPanel` | Phase-continuation actions live in the composer completion surface. Minimap renders one chip per **session** (`MinimapPanel`), not per message inside a session; this plugin has no per-message timeline to chip. |
...
| UI surfaces | Thread header: phase pill, status, context gauge, context-warning banner, Proceed/Iterate/Fork/Interrupt | full (superset) | `ui/rpi.tsx` `RpiThreadHeaderAction`, `RpiComposerBanner`, `app.tsx` `composer.customize` | The header keeps session controls; Review code, Create pull request, and Resolve pull request reviews are offered from the composer when phase completion makes them relevant. |
...
| UI surfaces | Sidebar `experimental_threadList` replacement: groups this plugin's task session threads by task with a phase pill; completed sessions and other threads both support selection and bulk archive; a manual "Use default list" toggle renders `Original` | full, shipped last/optional | `ui/rpi.tsx` `RpiThreadList`, `app.tsx` | Uses bb's host-owned `experimental_useSidebarThreadActions().archive()`, which archives children and repairs open panes. The SDK has no bulk-delete action, and single-thread deletion remains in bb's native confirmation flow. |
